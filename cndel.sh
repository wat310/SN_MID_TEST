...
SCRIPT_NAME='cndel.sh' # 実行スクリプト名
RG_TYPE='cn'
RG_STATUS='del'
RG_FILE_NAME='CnDel_Snw.csv'
...
inotifywait -m -q -e create $EXPORT_DIR | while read line; do
  ...
  # csv整形(ヘッダー削除、ファイル作成、/home/rguser/(CONVERTED)に配置)
    ...
    # RG出力用データ作成
    # r3_contract_number,asset_tag,sys_id,asset_class が飛んでくる想定
    ...
    do
        ...
        if [ $(echo -n $line | sed -e 's@[^,]@@g' | wc -c) != 3 ];then
        ...
        #  小文字は大文字に変換する
        asset_class=$(echo $line | cut -d ',' -f 4 | sed -e "s/\"//g")
        ...
        # RG出力用データ
        echo "${cn}" >>"${ARCHIVE_DIR}rg_${current_time_2}.csv"
