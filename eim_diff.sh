...
# サーバー外からのファイル配置はCREATEイベントが反応する
...
table_name='x_ritsc_a200002102_import_eim' # テーブル名
...
script_name='eim_diff.sh' # 実行スクリプト名
...
inotifywait -m -q -e create,moved_to $IMPORT_DIR | while read line; do
    ...
    # diff_updateが来た場合
    if [[ $file_name =~ $file_pattern_diff_update ]] ; then
        # 移動してあれば処理済みなので処理終了
        ...
        # timestampを抽出
        timestamp=`echo "$file_name" | sed -e "s/^eim_diff_update_//g" | sed -e "s/.csv$//g"`

        # もう一方のfile_name
        other_file_name="eim_diff_delete_${timestamp}.csv"
    ...
    # diff_deleteが来た場合
    elif [[ $file_name =~ $file_pattern_diff_delete ]] ; then
        # 移動してあれば処理済みなので処理終了
        if [[ -f "${ORIGIN_DIR}${file_name}" ]] ; then
            log_info "${file_name}は処理済みのため、処理を終了し、監視を継続します。"
            continue
        ...
        # timestampを抽出
        timestamp=`echo "$file_name" | sed -e "s/^eim_diff_delete_//g" | sed -e "s/.csv$//g"`

        # もう一方のfile_name
        other_file_name="eim_diff_update_${timestamp}.csv"
    ...
    # full_update
    elif [[ $file_name =~ $file_pattern_full_update ]] ; then
        # ログをだして終了する
        log_error "EIMのfullデータを検知しました。運用手順書に沿って対応してください。対象データ：${file_name}"
            # ★メール通知
            send_mail "EIMのfullデータを検知しました。運用手順書に沿って対応してください。対象データ：${file_name}" "【EIM FULL】デバイス認証 ${mid_host} ${script_name}"
        log_info "処理を終了し、監視を継続します。"
        continue
    ...
    # full_delete
        ...
        log_debug "${file_name}はファイル名がインポートファイルのパターンとマッチしません。"
        log_info "処理を終了し、監視を継続します。"
        continue
    fi
    ...
    # 存在しなければ、単体で処理を行う
        ...
        log_debug "もう片方のファイルを確認出来ませんでした。単独で処理を実行します。"
        log_debug "${file_name}について、flagの追記をします。"
        # フラグを追記
        cp -p $ORIGIN_DIR$file_name $DATA_DIR
